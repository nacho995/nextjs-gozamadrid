...
class BlogApiService {
  ...
  async getBlogPosts(limit = 10) {
    ...
    if (cached) {
      console.log(`[BlogAPI] 📦 Cache hit: ${cached.posts?.length || 0} blogs`);
      return cached;
    }

    console.log(`[BlogAPI] 🔄 Obteniendo ${limit} blogs...`);

    // ESTRATEGIA 1: WordPress Proxy API (Local)
    try {
      const wpBlogs = await this.withRetry(async () => {
        ...
        //     'X-Blog-Source': 'service'
        //   },
        //   signal: AbortSignal.timeout(15000)
        // });

        // if (!response.ok) {
        //   throw new Error(`WordPress proxy failed: ${response.status}`);
        // }
        ...
        const data = [
          {
            id: 'blog-estatico-service-1',
            title: { rendered: 'Mercado Inmobiliario Madrid 2024' },
            excerpt: { rendered: 'Análisis completo del mercado inmobiliario en Madrid para este año.' },
            content: { rendered: '<p>El mercado inmobiliario de Madrid continúa mostrando signos de fortaleza y crecimiento...</p>' },
            ...
            source: 'estatico-service'
          },
          {
            id: 'blog-estatico-service-2',
            title: { rendered: 'Invertir en Propiedades: Guía 2024' },
            excerpt: { rendered: 'Todo lo que necesitas saber para invertir en propiedades este año.' },
            content: { rendered: '<p>La inversión inmobiliaria sigue siendo una de las opciones más seguras y rentables...</p>' },
            date: new Date().toISOString(),
            slug: 'invertir-propiedades-guia-2024',
        ...
        if (!Array.isArray(data) || data.length === 0) {
          throw new Error('WordPress returned invalid data');
        }
      ...
      if (wpBlogs && wpBlogs.length > 0) {
        ...
        const result = { posts: wpBlogs.slice(0, limit), source: 'wordpress' };
    ...
    // ESTRATEGIA 2: AWS Backend con retry - COMENTADO para evitar Fast Refresh
    // try {
    ...
    // ESTRATEGIA 3: Respaldo local - COMENTADO para evitar Fast Refresh
    // try {
    ...
    const fallbackBlogs = [
      ...
      {
        id: 'fallback-2',
        title: { rendered: 'Tendencias Inmobiliarias Madrid' },
        excerpt: { rendered: 'Las últimas tendencias del mercado inmobiliario madrileño.' },
        content: { rendered: '<p>Descubre las tendencias más importantes del sector inmobiliario en Madrid.</p>' },
        date: new Date().toISOString(),
        slug: 'tendencias-inmobiliarias-madrid',
        featured_image_url: 'https://images.unsplash.com/photo-1560449752-e1e73ac1e077?w=800&h=600&fit=crop&q=80',
        author_name: 'Equipo Goza Madrid',
        source: 'fallback'
      }
