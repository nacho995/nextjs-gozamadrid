...
const Video = () => {
    ...
    // Hook optimizado para propiedades REALES con cache y retry - PRIORIDAD A MONGODB
    const { 
        ...
        limit: 33,
        source: 'mongodb', // Cambiar a 'mongodb' para dar prioridad a datos reales del backend
        autoLoad: true,
        enableCache: true,
        skipInitialLoad: false
    });

    // Hook adicional para cargar todas las fuentes como fallback
    const { 
        ...
        limit: 33,
        source: 'all',
        autoLoad: !allProperties.length && !propertiesLoading, // Solo cargar si no hay propiedades de MongoDB
        enableCache: true,
        skipInitialLoad: false
    });

    // Combinar propiedades de MongoDB y fallback
    const combinedProperties = useMemo(() => {
        if (allProperties.length > 0) {
            console.log(`[Video] 🎯 Usando ${allProperties.length} propiedades de MongoDB`);
            return allProperties;
        } else if (fallbackProperties.length > 0) {
            console.log(`[Video] 🔄 Usando ${fallbackProperties.length} propiedades de fallback`);
    ...
    useEffect(() => {
        const isLoading = propertiesLoading || fallbackLoading;
        const hasError = propertiesError || fallbackError;
        
        if (!isLoading && combinedProperties.length === 0 && hasError) {
            console.log('🏠 No hay propiedades disponibles de ninguna fuente');
            setShowNoPropertiesMessage(true);
        } else if (combinedProperties.length > 0) {
            setShowNoPropertiesMessage(false);
            console.log(`[Video] 📊 Total propiedades disponibles: ${combinedProperties.length}`);
            console.log(`[Video] 📊 Fuentes MongoDB: ${meta?.sources?.mongodb || 0}, Ejemplo: ${meta?.sources?.ejemplo || 0}`);
        }
    }, [combinedProperties, propertiesLoading, fallbackLoading, propertiesError, fallbackError, meta]);

    // Efecto para seleccionar la primera propiedad cuando se cargan
    useEffect(() => {
        if (combinedProperties.length > 0 && !selectedProperty) {
            const normalizedProperties = combinedProperties.map(normalizeProperty);
            ...
            console.log('[Video] 📊 Total propiedades cargadas:', combinedProperties.length);
            console.log('[Video] 📊 Fuente:', normalizedProperties[0]?.source || 'Desconocida');
        }
    }, [combinedProperties, selectedProperty]);

    // Función para filtrar propiedades usando la utilidad
    const getFilteredProperties = () => {
        return filterProperties(combinedProperties, searchFilters);
    };
                                                ...
                                                <div className="inline-flex items-center gap-2 text-white/80 text-sm">
                                                    {propertiesLoading ? (
                                                        ...
                                                        <span className="text-amarillo font-medium">
                                                            {combinedProperties.length} propiedades disponibles
                                                            {getFilteredProperties().length !== combinedProperties.length && (
                                        ...
                                        <div className="overflow-y-auto p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6">
                                            {propertiesLoading ? (
                                                ...
                                                <div className="text-center py-16 text-gray-600">
                                                    ...
                                                    <h3 className="font-serif text-2xl font-light mb-4">
                                                        {combinedProperties.length === 0 ? 'No hay propiedades disponibles' : 'No se encontraron propiedades'}
                                                    </h3>
