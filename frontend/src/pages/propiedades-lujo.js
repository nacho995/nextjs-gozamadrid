...
const PropiedadesLujo = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Mostrar popup después de 1500px de scroll si no está minimizado
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white relative">
        ...
        {/* Popup que aparece después de hacer scroll */}
        <AnimatePresence>
          {showPopup && (
            <motion.div
              className="fixed bottom-0 inset-x-0 z-50 mb-0 p-4 flex justify-center items-center"
              initial={{ y: 100, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              exit={{ y: 100, opacity: 0 }}
            >
              <div className="bg-white rounded-lg shadow-2xl p-8 max-w-md w-full border-2 border-yellow-500 relative backdrop-blur-md bg-white/95">
                <button 
                  onClick={minimizePopup}
                  className="absolute top-2 right-2 text-gray-500 hover:text-yellow-600 flex items-center gap-1 text-sm font-medium px-2 py-1 rounded-full hover:bg-gray-100 transition-colors"
                ...
                <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-gradient-to-r from-yellow-400 to-yellow-700 flex items-center justify-center">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-black" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                  </svg>
                </div>
                <h3 className="text-2xl font-bold mb-3">¿Sabe cuánto vale su propiedad?</h3>
                <p className="mb-6 text-gray-700">Descubra el valor real de su propiedad de lujo en Madrid con nuestra tasación profesional gratuita y personalizada.</p>
                <div className="flex justify-center">
                  <a 
                    href={valorationLink}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="bg-gradient-to-r from-yellow-500 to-yellow-700 text-black px-8 py-3 rounded-full font-bold hover:from-yellow-600 hover:to-yellow-800 transition-all duration-300 shadow-lg w-full block text-center"
                  >
                    Solicitar Valoración Gratuita
                  </a>
                </div>
          ...
          {isPopupMinimized && (
            <motion.button
              ...
              className="fixed bottom-8 left-8 z-50 bg-gradient-to-r from-yellow-500 to-yellow-700 text-black px-4 py-2 rounded-full shadow-lg flex items-center space-x-2 border border-yellow-400/30"
              initial={{ y: 100, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              exit={{ y: 100, opacity: 0 }}
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              style={{ bottom: isPopupMinimized ? '5rem' : '2rem' }}
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
