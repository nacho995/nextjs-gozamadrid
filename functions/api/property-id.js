/**
 * Manejador alternativo para propiedades usando query string
 * Este script permite acceder a propiedades específicas a través de 
 * /api/property-id?id=XXX en lugar de rutas dinámicas
 */
...
export async function onRequest(context) {
  ...
  console.log('PropertyID: Solicitud recibida', {
    url: request.url,
  ...
  try {
    // Obtener ID de propiedad de la URL (query string)
    const url = new URL(request.url);
    const propertyId = url.searchParams.get('id');
    
    if (!propertyId) {
      console.error('PropertyID: ID de propiedad no proporcionado en query string');
      
      headers.set('Content-Type', 'application/json');
      return new Response(JSON.stringify({ 
        error: 'ID de propiedad no proporcionado',
        usage: 'Utilice /api/property-id?id=XXXX donde XXXX es el ID de la propiedad'
      }), { status: 400, headers });
    }
    
    console.log('PropertyID: ID de propiedad solicitada:', propertyId);
    ...
    if (propertyCache[propertyId]) {
      ...
      if (cacheAge < 300000) {
        console.log('PropertyID: Usando datos en caché para:', propertyId);
    ...
    const source = url.searchParams.get('source') || (isMongoDBId ? 'mongodb' : 'unknown');
    ...
    try {
      if (isMongoDBId || source === 'mongodb') {
        ...
        console.log('PropertyID: Intentando conectar a API en:', apiEndpoint);
        ...
        try {
          ...
          console.log('PropertyID: Error en endpoint principal, probando respaldo:', mainError.message);
          ...
          try {
            ...
            console.error('PropertyID: Error también en el endpoint de respaldo:', backupError.message);
        ...
        try {
          ...
          console.error('PropertyID: Error al consultar propiedades normales:', error.message);
          apiError = `No se pudo conectar a la API: ${error.message}`;
        }
      }
    } catch (fetchError) {
      console.error('PropertyID: Error global de fetch:', fetchError.message);
      apiError = `Error de conexión: ${fetchError.message}`;
    }
    
    // Si no se pudo obtener la propiedad, crear una de demostración
    if (!property) {
      console.log('PropertyID: Generando propiedad de demostración para:', propertyId);
    ...
    if (cacheKeys.length > 50) {
      ...
      console.log('PropertyID: Caché limpiada, eliminada entrada:', oldestKey);
    }
    ...
    console.error('PropertyID: Error general:', error);
